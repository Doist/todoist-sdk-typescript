...
function main() {
    ...
    const languages = ghJson(`contents/dist/json?ref=${REF}`)
    ...
    for (const language of languages) {
        const byToken = new Map(
            ghFile(`dist/json/${language}.json`).tokenRegexDef.map((entry) => [
                entry.name,
                entry.regex,
            ]),
        )
...
function renderConformance({ sha, languages }) {
    const grouping = ghFile('test_json/grouping_test.json')
    const rows = []

    for (const language of languages) {
        const seenTokens = new Set()
        const seenQueries = new Set()
        for (const testCase of ghFile(`test_json/lexer_i18n_tests/${language}_test.json`)) {
